...
import web.modelo.services.UsuarioDetallesServiceImpl;


/**
 * Este filtro se ejecuta en cada solicitud que llega a la aplicación.
 *
 * Lo que hago hecho aquí es revisar si la solicitud lleva un token JWT válido en el encabezado.
 * 
 * Paso a paso:
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    // Rutas públicas que no deben pasar por la validación JWT ---- "/curso", "/evento", "/via", "/planificacion",
    private static final List<String> PUBLIC_PATHS = List.of(
    	    "/swagger-ui", "/swagger-ui.html", "/v3/api-docs", "/swagger-resources",
    	    "/webjars", "/favicon.ico", "/auth/login", "/auth/registro",
    	     "/aforo"
    	);
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        // Si la ruta es pública, no aplicamos validación JWT
